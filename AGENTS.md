...
## Roo Code / Zoo Code 迁移

本项目已迁移到 Roo Code / Zoo Code 平台，目标平台文件位于 `roocode/` 目录。

迁移规则详见 [`docs/migration-guide.md`](./docs/migration-guide.md)，迁移记录见 [`roocode/migration-logs/`](./roocode/migration-logs/)。

执行迁移工作前必须先阅读 [`docs/migration-guide.md`](./docs/migration-guide.md) 理解完整规则。对 `skills/` 源目录的任何修改（新增、更新、删除），必须同步到 `roocode/`，遵循迁移规则，并在完成后于 `roocode/migration-logs/` 中创建增量同步记录。
