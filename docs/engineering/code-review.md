...
Reach for this when there is a diff to judge against a known-good point and you want the two questions — *is it built right?* and *is it the right thing?* — answered independently. It runs at the end of the build loop, in a fresh session after [implement](https://aihero.dev/skills-implement) has committed; for actually writing the code test-first, use [tdd](https://aihero.dev/skills-tdd), and for building a whole spec into code use `implement`, which commits first and then points you here.
...
Its closest neighbour is [implement](https://aihero.dev/skills-implement), which drives the build, commits, and then points the user here in a fresh session; this review's close-out closes the loop — updating the originating issue, ADR status, and parent tracking once the work passes. Upstream, the spec it checks against is produced by [to-spec](https://aihero.dev/skills-to-spec) and [to-tickets](https://aihero.dev/skills-to-tickets). When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
