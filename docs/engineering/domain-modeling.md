...
The skill writes into two places, both created lazily — only once there is something to record. Resolved terms go into the domain glossary (path from the Workflow paths table — `CONTEXT.md` at the root in the Distributed layout, or under `matt-workflow/domain/` in Centralized; in a multi-context repo, into the per-context glossary). Decisions go into the ADR directory (also from the Workflow paths table). Nothing needs to exist up front; the first resolved term creates the glossary, the first real trade-off creates the ADR.
...
- **An ADR** captures a decision, and the bar is high: offered only when the choice is **hard to reverse**, **surprising without context**, and **the result of a real trade-off**. Miss any one of the three and there is no ADR. This is what keeps the ADR directory a record of consequential forks rather than a diary.
