...
`implement` builds the work described in a spec or a set of tickets — driving it through test-driven development, typechecking, and the full test suite, then committing to the current branch and handing off to review in a fresh session.
...
Around that core it keeps the loop tight — typecheck often, run single test files as it goes, run the whole suite once at the end — then records a review base, commits to the current branch, and tells the user to run `/code-review` in a fresh session. It does not update the originating issue or ADRs itself; that close-out belongs to the review once the work has passed.
