...
- **Local files** → one file per ticket in the local issues directory (path from the Workflow paths table), numbered blockers-first, the edges written as text. You work them top-to-bottom, by hand, staying in the loop.
...
It sits between [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec with user stories to slice against, and [implement](https://aihero.dev/skills-implement), which builds each ticket, driving [tdd](https://aihero.dev/skills-tdd) internally to write the tests test-first, before committing and handing off to [code-review](https://aihero.dev/skills-code-review) in a fresh session. If the breakdown came from a parent issue, it posts a tracking checklist to it. Work the frontier one ticket per fresh context, clearing between them. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
