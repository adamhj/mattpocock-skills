...
The issue tracker and triage label vocabulary should have been provided to you — tell the user to run `/setup-matt-pocock-skills` if not. Artifact paths are in the Workflow paths table in `CLAUDE.md`/`AGENTS.md`.
...
- **Local files** → write one file per ticket under the local issues directory (`<scratch>/<feature-slug>/issues/<NN>-<slug>.md`, path from the Workflow paths table), numbered from `01` in dependency order (blockers first). Each file's "Blocked by" lists the numbers/titles it depends on. Use the per-ticket file template below — one ticket per file, never a single combined file.
...
Tell the user to work the frontier one ticket at a time with `/implement`, clearing context between tickets.
