...
The issue tracker should have been provided to you — tell the user to run `/setup-matt-pocock-skills` if the issue tracker config file (path in the Workflow paths table) is missing.
...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the workflow in the issue tracker config file.
2. A path the user passed as an argument.
3. A PRD/spec file under `docs/`, `specs/`, or the local issues directory (path in the Workflow paths table) matching the branch name or feature.
...
- Create a fix issue referencing the original issue, then tell the user to run `/implement` in a new session, or
